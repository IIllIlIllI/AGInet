...
ROOT="${1:-$(pwd)}"
BACKUP_DIR="$ROOT/.paste-backups/agi-loop-ambiguity-glyph-$(date +%Y%m%d-%H%M%S)"
...
echo "[patch] adding ambiguity glyph to AGInet loop"
...
def snapshot_from_report(report: dict | None) -> dict | None:
    ...
    return {
        ...
        "residual_ambiguity": result.get("residual_ambiguity"),
...
def status_glyph_line(report: dict | None, history: deque) -> str:
    ...
    ambiguity = result.get("residual_ambiguity", 0)
    ...
    try:
        ...
        a_val = 0
    a_state = "red" if a_val >= 3 else "yellow" if a_val >= 2 else "green"
...
def render(
    ...
    lines.append(f"residual_ambiguity:    {ambiguity_text(result.get('residual_ambiguity'))}")
    ...
    if history:
        ...
        lines.append("  run_id                variant                  top            score    amb  sovr     contam")
        prev_top = None
        for item in history:
            ...
            amb = ambiguity_text(item.get("residual_ambiguity"))
            ...
            lines.append(
                ...
                f"{str(score):<8} {amb:<8} {sovr:<16} {contam:<10}"
            )
...
echo '  git add sim/tools/loop_runner.py'
echo '  git commit -m "add ambiguity glyph to AGInet loop"'
