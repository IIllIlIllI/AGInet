...
ROOT="${1:-$(pwd)}"
BACKUP_DIR="$ROOT/.paste-backups/agi-status-full-$(date +%Y%m%d-%H%M%S)"
...
echo "[patch] adding full mode to AGInet status command"
...
echo "  tools/bin/agi status --full"
echo
echo "Suggested commit:"
echo '  git add tools/bin/agi sim/tools/status_summary.py'
echo '  git commit -m "add full mode to AGInet status command"'
