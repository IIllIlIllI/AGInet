...
ROOT="${1:-$(pwd)}"
BACKUP_DIR="$ROOT/.paste-backups/agi-loop-colors-$(date +%Y%m%d-%H%M%S)"
...
echo "[patch] adding colorized status markers to AGInet loop"
...
echo
echo "Optional:"
echo "  AGINET_NO_COLOR=1 tools/bin/agi loop"
echo
echo "Suggested commit:"
echo '  git add sim/tools/loop_runner.py'
echo '  git commit -m "add colorized status markers to AGInet loop"'
