...
ROOT="${1:-$(pwd)}"
BACKUP_DIR="$ROOT/.paste-backups/agi-watch-$(date +%Y%m%d-%H%M%S)"
...
echo "[patch] adding AGInet watch dashboard"

write_file "$ROOT/sim/tools/watch_dashboard.py" <<'EOF'
...
echo "  tools/bin/agi watch"
echo
echo "Suggested commit:"
echo '  git add tools/bin/agi sim/tools/watch_dashboard.py install_aginet_aliases.sh'
echo '  git commit -m "add AGInet watch dashboard"'
